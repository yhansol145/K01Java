...
다형성(Polymorphism)
	...
	4. 이런경우는 이질화(Heterogeneous:헤테로지니어스)라고 한
	
	- 같은 타입의 클래스로 객체를 참조
	1. 객체 전체를 접근할 수 있다. 즉 클래스의 일반적인
		규칙이 적용된다.
	2. 이런경우는 동질화(Homegeneous:호모지니어스)라고 한다.
 */
class Parent {
	int parentMember;
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		parent1.parentMethod(); //오버라이딩 한 메소드이므로 자식쪽이 호출된다.
		...
		/*
		자식쪽 멤버에 접근하는 방법1 : 다운캐스팅 후 바로 접근한다.
		 */
		...
		상속한다. 따라서 모든 객체는 Object의 참조변수로 참조가 가능하다.
		또한 Object클래스에 정의된 모든 메소드를 별도의
		선언없이 사용할 수 있다.
		 */
