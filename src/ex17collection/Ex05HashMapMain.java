	...
	public static void main(String[] args) {
		...
		/*
			2. 키값을 모를때
				...
				얻어온다. 해당 반환값을 확장for문을 통해 추출하고, 이를
				통해 값을 출력할 수 있다.
		...
		while(it.hasNext()) {
			String key = it.next();
			String value = map.get(key);
			System.out.println(String.format("%3:%3", key, value));
		}
		 ...
		 */
		System.out.println(map.containsKey("name") ? "name키갑있다" : "name키값없다");
		System.out.println(map.containsKey("account") ? "account키갑있다" : "account키값없다");
		...
		//전체삭제 : removeAll()은 없음. clear()만 사용 가능함.
		map.clear();
